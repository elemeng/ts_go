...
export const cacheWarning = derived([currentCacheSize, indexedDbCacheSize], ([$currentCacheSize, $indexedDbCacheSize]) => ({
...
// 获取 PNG
export async function getPng(tsId: string, zIndex: number, bin = 8, quality = 90): Promise<Blob | null> {
...
function putPngToMemory(key: string, data: Blob): void {
	...
	let cacheSize = 0
	currentCacheSize.subscribe(n => { cacheSize = n })()
...
// 存入 IndexedDB
export async function putPng(tsId: string, zIndex: number, data: Blob, bin = 8, quality = 90): Promise<void> {
	const key = cacheKey(tsId, zIndex, bin, quality)
...
function debouncePersist(): void {
	...
	persistTimeout = setTimeout(() => {
		...
		const unsubscribe = selections.subscribe(s => { state = s })
		unsubscribe()
...
export async function fetchPng(tsId: string, zIndex: number, bin = 8, quality = 90): Promise<Blob> {
	const response = await fetch(`${API_BASE}/api/preview/${tsId}/${zIndex}?bin=${bin}&quality=${quality}`)
