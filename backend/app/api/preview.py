...
async def get_preview(ts_id: str, frame_id: int, bin: int = 8, quality: int = 90):
    ...
    try:
        ...
        print(f"[get_preview] Request: ts_id={ts_id}, frame_id={frame_id}, bin={bin}, quality={quality}")
        ...
        with _inflight_lock:
            ...
            if task_key in _inflight_tasks:
                ...
                print(f"[get_preview] Waiting for existing task: {task_key}")
            ...
            print(f"[get_preview] Created new task: {task_key}")
        ...
        try:
            ...
            if cached:
                ...
                print(f"[get_preview] Memory cache hit: {task_key}")
            ...
            print(f"[get_preview] Checking disk cache: {png_path}")


            if png_path.exists():


                print(f"[get_preview] Disk cache hit: {png_path}")
            ...
            print(f"[get_preview] Cache miss, generating PNG: {task_key}")
            ...
            if not ts:
                ...
                print(f"[get_preview] Tilt series not found: {ts_id}")
            ...
            print(f"[get_preview] Found tilt series: {ts_id} with {len(ts.frames)} frames")
            ...
            if not frame:
                ...
                print(f"[get_preview] Frame not found: {frame_id} in tilt series {ts_id}")


                print(f"[get_preview] Available frames: {[f.zIndex for f in ts.frames]}")
            ...
            print(f"[get_preview] Found frame: {frame_id}, mrcPath: {frame.mrcPath}")
            ...
            if img is None:
                ...
                print(f"[get_preview] Failed to read image: {frame.mrcPath}")
            ...
            print(f"[get_preview] Read image shape: {img.shape}")
            ...
            if bin > 1:
                ...
                print(f"[get_preview] Binned image shape: {img.shape}")
            ...
            print(f"[get_preview] Saved PNG to: {png_path}")
            ...
            print(f"[get_preview] PNG size: {len(data)} bytes")
            ...
            print(f"[get_preview] Successfully generated PNG: {task_key}")
            ...
            with _inflight_lock:
                ...
                if task_key in _inflight_tasks:
                    ...
                    print(f"[get_preview] Cleaned up task: {task_key}")
        ...
        print(f"[get_preview] Exception: {str(e)}")
