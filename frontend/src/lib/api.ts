...
export async function getTiltSeries(tsId: string): Promise<TiltSeries | null> {
  try {
    const response = await fetch(`${API_BASE}/api/mdoc/${encodeURIComponent(tsId)}`);
...
export async function listDirectory(path: string): Promise<{ name: string; type: 'dir' | 'file' }[]> {
...
export async function loadConfig(filename: string): Promise<ScanConfig> {
  const response = await fetch(`${API_BASE}/api/files/load-config?filename=${encodeURIComponent(filename)}`);
...
/** Fetch a PNG preview for a frame */
export async function fetchPng(tsId: string, zIndex: number, bin = 8, quality = 90): Promise<Blob> {
  const response = await fetch(
    `${API_BASE}/api/preview/${encodeURIComponent(tsId)}/${zIndex}?bin=${bin}&quality=${quality}`
  );
